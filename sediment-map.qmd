...
value_range_result = db.query(`
  ...
  FROM (
    SELECT
      ...
      AVG(
        CASE
          WHEN s.unit = 'mg/kg' THEN s.value
          WHEN s.unit = 'ug/g' THEN s.value
          WHEN s.unit = 'ug/kg' THEN s.value / 1000
          WHEN s.unit = 'g/kg' THEN s.value * 1000
          ELSE NULL
        END
      ) AS avg_value
    FROM sediment s
    ...
    JOIN project p  ON p.project_id = s.project_id
    WHERE s.param = '${selected_element.value}'
      AND s.unit IN ('mg/kg', 'ug/g', 'ug/kg', 'g/kg')
  ...
  if (legendEl) {
    ...
    legendEl.innerHTML = `
      ...
      <div style="font-size:11px;color:#555">
        Avg (mg/kg) · ${year_start}–${year_end} · ${countryLabel}
      </div>
...
map_data.length === 0
  ...
  : md`**${selected_element.label}** · ${year_start}–${year_end} · ${selected_country} · **${map_data.length}** locations · values in mg/kg`
```
