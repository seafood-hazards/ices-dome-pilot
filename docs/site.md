...
- **DataTables** — paginated table rendering on the code-lookup and grain-size pages
...
- `ices_dome_pilot.sqlite` from the **latest** release
- sql.js and stratum-sqlite into `libs/sqljs/`

Both are gitignored, and both are skipped if the file is already there — so a
local render after a database change needs the old copy deleted first. Do not
hand-edit anything under `libs/`.

CI/CD is `.github/workflows/publish.yml`: push to `main` renders the site and
deploys `_site/` to GitHub Pages. Local and CI render the same way.
...
| `data-preparation.qmd` | How the raw ICES-DOME export was transformed before import |
| `sediment-fractions.qmd` | Grain Size Codes — the matrix and parameter code vocabulary |
...
| `code-lookup-browser.qmd` | Browser for the ICES code vocabularies in `code_lookup` |
...
longer exports a dataset file, and `data-export.qmd` was removed at v0.1.19.

## Git workflow

Gitflow (`main`, `develop`, `feature/*`, `release/*`). No pull requests — merge
feature branches directly into `develop` and delete them when done. Do not
propose opening a GitHub PR unless explicitly asked.

## Releasing

**Every release must carry `ices_dome_pilot.sqlite` as an asset.** The site
...
5. **Create the GitHub release and upload the database, before pushing `main`:**
   ...
   gh release create vX.Y.Z --title vX.Y.Z --notes-from-tag \
     ../multised-engine/data/db/ices_dome_pilot.sqlite
   ```
